var driveHandler = new function() {
    ...
    this.load = function() {
      ...
      document.getElementById("cdown").innerHTML = "Exit pit stop to begin";
      document.getElementById("pitStopButton").onclick = pitStop;
...
function readTextFile(file)
{
    ...
    rawFile.onreadystatechange = function ()
    {
        if(rawFile.readyState === 4)
        {
            ...
            {
                carName = rawFile.responseText;
                document.getElementById('output').innerHTML = carName;
                document.getElementById('title').innerHTML = carName;
            }
        }
    }
    rawFile.send(null);
...
function notifyCharging() {
  ...
  var noticitationString = "Needs Charge: " + carName;
  ...
  // Let's check whether notification permissions have already been granted
    ...
    var notification = new Notification(noticitationString);
  }
  ...
  // Otherwise, we need to ask the user for permission
    ...
    Notification.requestPermission(function (permission) {
      ...
      if (permission === "granted") {
        var notification = new Notification("Charge: " + noticitationString);
      }
    });
  }
...
var batteryTime = 1 // Battery lasts for 30 mins
...
function resetTimer(){
  ...
  countDownDate = addMinutes(countDownDate, batteryTime);
...
var x = setInterval(function() {
  ...
  document.getElementById("cdown").innerHTML = minutes + "m " + seconds + "s ";

  // If the count down is finished, write some text 
  if (distance < 0) {
    clearInterval(x);
    document.getElementById("cdown").innerHTML = "OUT OF CHARGE!";
    notifyCharging();
  } else if (inPitStop){
    clearInterval(x);
    document.getElementById("cdown").innerHTML = "Exit pit stop to begin";
...
function pitStop(){
    ...
    var updateUI = function() {
      ...
      if (inPitStop){
        document.getElementById("pitStopButton").setAttribute("value", "Continue racing")
        document.getElementById("resetButton").style.visibility = "hidden";
        document.getElementById("subcdown").innerHTML = "The car is now in a pit stop. Replace the battery, put the car back on the track and press 'continue racing' when ready."
      }
      else{
        document.getElementById("pitStopButton").setAttribute("value", "Enter pit stop")
        document.getElementById("resetButton").style.visibility = "visible";
        document.getElementById("subcdown").innerHTML = "Remaining until car needs to enter pit stop for charging"
      }
    ...
    var postDrive = function() {
        ...
        console.log(data)
        document.getElementById("commandDisplay").innerHTML = data
        $.post(driveURL, data)
        updateUI()
    };
      ...
      for (var i = 0; i < gamepads.length; ++i)
        {
          ...
          if (pad && pad.timestamp!=0)
          {
            ...
            // Default gamepad axes are configured to that of a playstation controller
            var xAxisIndex = 2;
            var yAxisIndex = 1;

            console.log("You are using a " + pad.id);

            // Change horizontal axis index for our joystick controller
            if (pad.id == "Saitek ST90 USB Stick (Vendor: 06a3 Product: 0d62)"){
              xAxisIndex = 0;
            }

            var joystickX = applyDeadzone(pad.axes[xAxisIndex], 0.05);
            var joystickY = applyDeadzone(pad.axes[yAxisIndex], 0.15);
    ...
    var enterPit = function(i){
      console.log('post drive: ' + i)
      state.tele.user.angle = 0
      state.tele.pilot.angle = 0
      state.tele.user.throttle = state.maxThrottle
      state.recording = false
      postDrive()
      ...
      if (i < 4) {
        ...
        setTimeout(function () {
          console.log('calling pit stop:' + i)
          enterPit(i);
        }, 500)
      };
